...
public class ProductIngestionService {
    ...
    /**
     * Call this whenever a product is created or updated. Builds the embedding text
    ...
    public void indexProduct(Integer id) {
        ...
        String categoryNames = product.getCategories()
            .stream()
        ...
        Document doc = new Document(
            ...
            Map.of(
                "productId",
                product.getId().toString(), // always store as String in metadata
                "name",
                product.getName(),
                "categories",
                categoryNames));
    ...
    /**
     * Remove a product from the vector store (on hard delete or when toggling
     * deleted=true). The document ID in PgVector is derived from the productId
     * metadata — we use a filter-based delete here since we stored productId in
     * metadata.
     */
