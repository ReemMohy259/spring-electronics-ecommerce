...
public class IntentClassifier {
    ...
    private static final List<Pattern> PRODUCT_PATTERNS = List.of(
        Pattern.compile(
            "\\b(show|find|recommend|suggest|search|look for|get)\\b",
            Pattern.CASE_INSENSITIVE),
        Pattern.compile(
            "\\b(product|item|buy|purchase|price|cost|cheap|expensive|budget)\\b",
            Pattern.CASE_INSENSITIVE),
        Pattern
            .compile("\\b(best|top|good|latest|available|in stock)\\b", Pattern.CASE_INSENSITIVE),
        Pattern.compile(
            "\\b(laptop|phone|camera|headphone|tv|tablet|watch)\\b",
            Pattern.CASE_INSENSITIVE)
    ...
    private boolean fallbackKeywordMatch(String message) {
        return PRODUCT_PATTERNS.stream().anyMatch(pattern -> pattern.matcher(message).find());
    }
}
