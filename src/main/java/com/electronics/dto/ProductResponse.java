...
public record ProductResponse(Integer id, Integer merchantId, String name, String description,
    ...
    public static ProductResponse from(Product product) {
        List<CategoryResponse> categories = product.getCategories().stream()
                ...
                .toList();
        return new ProductResponse(product.getId(), product.getMerchant().getId(),
                product.getName(), product.getDescription(), product.getPrice(),
                product.getStockQuantity(), product.getSoldUnits(), product.getSku(),
                product.getImageUrl(), product.getAdditionalInfo(), product.getCreatedAt(),
                categories);
    }
}
